...
function ScrewInit (options) {
    if (!(this instanceof ScrewInit)) return new ScrewInit(options);
...
ScrewInit.prototype.download = function (id, cb, progress) {
    log.notice('will download component id: ' + id);
    if (!id) {
        log.error(new Error('invalid, '));
    }
    var supports = ['github', 'gitlab'];
...
ScrewInit.prototype.replace = function (path, map, use_prompt, callback) {
...
ScrewInit.prototype.prompt = function (schema, cb) {
...
ScrewInit.prototype._roadmap = function (roadmap) {
    var self = this;
...
ScrewInit.prototype.deliver = function (from, to, roadmap) {
    to = to || '';
...
ScrewInit.prototype.release = function (id, to, replacer, roadmap, cb) {
    var that = this;
...
lodash.extend(ScrewInit.prototype, lodash);

module.exports = ScrewInit;
